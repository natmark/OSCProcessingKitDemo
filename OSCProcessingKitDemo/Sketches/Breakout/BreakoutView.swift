    ...
    func generate(frameWidth: CGFloat) -> [CGRect] {
        ...
        for x in 0..<Int(BLOCK_SIZE.x) {
            for y in 0..<Int(BLOCK_SIZE.y) {
                blocks.append(CGRect(x: x * Int(frameWidth / BLOCK_SIZE.x), y: y * BLOCK_HEIGHT, width: Int(frameWidth / BLOCK_SIZE.x), height: BLOCK_HEIGHT))
            }
        }
        return blocks
    }
...
class BreakoutView: ProcessingView {
    ...
    func setup(){
        stroke(0, 0, 0)
        bar = CGRect(x: 0, y: height - 50, width: 150, height: 20)
        ball = Ball(size: CGPoint(x: 20, y: 20), center: CGPoint(x: width / 2 - 200, y: bar.origin.y - 200) , velocity: CGPoint(x: 6, y: 6))
