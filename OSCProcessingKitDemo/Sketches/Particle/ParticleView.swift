...
class ParticleView : ProcessingView {
    let totalDots = 50
    var dots: [Dot] = []
    ...
    func setup() {
        ...
        for _ in 0..<totalDots {
            ...
            d.vx = CGFloat(Dot.getRandomNumber(Min: 0.0, Max: 2.0) - Float(1.0))
            d.vy = CGFloat(Dot.getRandomNumber(Min: 0.0, Max: 2.0) - Float(1.0))
            dots.append(d)
        }
...
class Dot {
    ...
    func update(){
        // update the velocity
        self.vx = self.vx + CGFloat(Dot.getRandomNumber(Min: 0.0, Max: 2.0) - 1.0)
        self.vx = self.vx * 0.96
        self.vy = self.vy + CGFloat(Dot.getRandomNumber(Min: 0.0, Max: 2.0) - 1.0)
    ...
    static func getRandomNumber(Min min : Float, Max max : Float) -> Float {
